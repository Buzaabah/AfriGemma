...
class EvalArguments:
    ...
    metric: Optional[str] = field(
        default='bleu',
        metadata={"help": "The metric used to evaluate the model, takes in value of bleu, rouge, meteor etc"}
    )
    compute_metric: Optional[bool] = field(
        default=False,
        metadata={"help": "whether to compute metrics while generating the outputs, must be False if num_samples > 1"}
    )
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[-1].endswith(".json"):
        ...
        model_args, data_args, eval_args, training_args = parser.parse_json_file(
    ...
    trainer = Seq2SeqTrainer(
        ...
        data_collator=Seq2SeqDataCollator(tokenizer, config.decoder_start_token_id, model_args.task_type,
                                          model_args.mode, data_args),
        tokenizer=tokenizer,
    )
    ...
    if model_args.mode == 'train':
        ...
        gen_eval(model, tokenizer, trainer.get_eval_dataloader(), training_args.output_dir,
                 data_args.test_max_target_length, eval_args, 'test')
